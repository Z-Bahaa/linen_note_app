<script lang="ts">
  ...
  let confirmAction: 'moveAllToTrash' | 'unarchiveAll' | null = null;
  ...
  function handleBulkAction(action: 'moveAllToTrash' | 'unarchiveAll') {
...
<header class="toolbar">
  <div class="toolbar-left">
    ...
    {#if $currentViewType === 'archived'}
      <button
        class="toolbar-button bulk-action"
        on:click={() => handleBulkAction('unarchiveAll')}
        title="Unarchive All"
      >
        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span class="label">Unarchive All</span>
      </button>

      <button
        class="toolbar-button bulk-action danger"
        on:click={() => handleBulkAction('moveAllToTrash')}
      ...
      >
        ...
        <span class="label">Move All to Trash</span>
      </button>
    {/if}
  </div>
  ...
  <ConfirmModal
    ...
    title={
      confirmAction === 'moveAllToTrash'
        ? 'Move All to Trash'
        : confirmAction === 'unarchiveAll'
        ? 'Unarchive All'
        : 'Confirm Action'
    }
    message={
...
<style>
  ...
  .toolbar-button.bulk-action {
    background-color: var(--color-bg-tertiary);
  }

  .toolbar-button.bulk-action:hover {
    background-color: var(--color-bg-elevated);
  }

  .toolbar-button.bulk-action.danger {
    color: var(--color-accent-red);
  }

  .toolbar-button.bulk-action.danger:hover {
